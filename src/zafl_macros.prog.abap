...
*&---------------------------------------------------------------------*
DEFINE /afl/log_init.
  DATA: /afl/xstr    TYPE xstring ,
        /afl/cx_root TYPE String ,
        /afl/json    TYPE string,
        /afl/encode  TYPE string ,
        funcdesc     TYPE tftit-stext.
  ...
  IF sy-subrc = 0.
    ...
    IF sy-subrc = 0.
      ...
      CLEAR : funcdesc .
      SELECT SINGLE tftit~stext INTO funcdesc FROM tftit
          WHERE tftit~funcname = /afl/func_name
...
DEFINE /afl/log_get_json.
  ...
  IF /afl/comp_tab IS NOT INITIAL.
    ...
    /afl/json = /ui2/cl_json=>serialize( data = </afl/parameter_data> ).
